...
export const Footer = () => {
  return (
    <motion.div>
      <footer className="footer footer-center z-0 w-dvw bg-secondary pb-5 pt-10 text-primary-content transition-transform top-oval sm:mt-10">
        <aside>
...
export const Layout = ({ children }: React.PropsWithChildren) => {
  const location = window.location.pathname; // Get current pathname from window

  const [theme, setTheme] = React.useState("autumn"); // dark, light, autumn, cake
  ...
  const firstChildrenWithStyles = React.cloneElement(childrenArray[0] as React.ReactElement, {
    injectedClassNames: "pt-5 md:pt-20 flex-1 transition",
  });
  const childrenWithInjectedStyles = [firstChildrenWithStyles, ...childrenArray.slice(1)];

  // Don't show if it's on any of these routes
  const blockedRoutes = ["/service/booking/"];
  ...
  return (
    <main className="relative flex min-h-[100vh] flex-col overflow-x-hidden" data-theme={theme}>
      <NavBar className="h-20 md:w-10/12">
        {routes.map(({ path, name }) => {
          if (blockedRoutes.includes(location)) return null;
